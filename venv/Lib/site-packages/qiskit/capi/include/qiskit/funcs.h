 ...
 * ``QkParam``.
 */
QkExitCode qk_circuit_parameterized_gate(QkCircuit *circuit,
                                         QkGate gate,
                                         const uint32_t *qubits,
                                         const QkParam *const *params);
 ...
 */
void qk_circuit_pauli_product_rotation(QkCircuit *circuit,
                                       const QkPauliProductRotation *rotation,
                                       const uint32_t *qubits);
 ...
 */
QkExitCode qk_circuit_inst_pauli_product_rotation(const QkCircuit *circuit,
                                                  size_t index,
                                                  QkPauliProductRotation *instruction);
 ...
 * * ``qubits`` is an aligned pointer to ``rotation->len`` initialized ``uint32_t`` values.
 */
void qk_circuit_pauli_product_measurement(QkCircuit *circuit,
                                          const QkPauliProductMeasurement *measurement,
                                          const uint32_t *qubits,
                                          uint32_t clbit);
 ...
 */
QkExitCode qk_circuit_inst_pauli_product_measurement(const QkCircuit *circuit,
                                                     size_t index,
                                                     QkPauliProductMeasurement *instruction);
...
#if (defined(QISKIT_C_PYTHON_INTERFACE) && defined(QISKIT_C_PYTHON_INTERFACE))
/**
 * @ingroup QkCircuit
 ...
 * writeable data of the correct type.
 */
int qk_classical_register_convert_from_python(PyObject *object,
                                              void *address);
#endif
 ...
 * behaviour.
 */
QkCircuit *qk_circuit_library_iqp(uint32_t num_qubits,
 ...
 */
QkCircuit *qk_circuit_library_suzuki_trotter(const QkObs *op,
                                             uint32_t order,
                                             uint32_t reps,
                                             double time,
                                             bool preserve_order,
                                             bool insert_barriers);
 ...
 */
uint32_t qk_dag_replace_block_with_unitary(QkDag *dag,
                                           uint32_t num_block_ids,
 ...
 * * `matrix` is not an aligned pointer to `4**num_qubits` initialized values,
 */
void qk_dag_substitute_node_with_unitary(QkDag *dag,
                                         uint32_t node,
                                         const QkComplex64 *matrix,
                                         uint32_t num_qubits);
 ...
 */
bool qk_transpiler_pass_check_gate_direction(const QkDag *dag,
                                             const QkTarget *target);
 ...
 * Behavior is undefined if ``dag`` or ``target`` are not valid, non-null pointers to ``QkDag`` and ``QkTarget`` objects, respectively.
 */
void qk_transpiler_pass_gate_direction(QkDag *dag,
                                       const QkTarget *target);
 ...
 * if ``target`` is not a valid pointer to a ``QkTarget``.
 */
Qk_DEPRECATED_FN_NOTE("use `qk_transpiler_pass_standalone_optimize_1q_sequences` instead")
void qk_transpiler_standalone_optimize_1q_sequences(QkCircuit *circuit,
                                                    const QkTarget *target);
 ...
 * if ``target`` is not a valid pointer to a ``QkTarget``.
 */
void qk_transpiler_pass_standalone_optimize_1q_sequences(QkCircuit *circuit,
                                                         const QkTarget *target);
...
/**
 * @ingroup QkTranspilerPasses
 * Run the RemoveIdentityEquivalent transpiler pass on a DAG circuit.
 *
 ...
 */
void qk_transpiler_pass_remove_identity_equivalent(QkDag *dag,
                                                   const QkTarget *target,
                                                   double approximation_degree);
 ...
 * Behavior is undefined if ``dag`` is not a valid, non-null pointer to a ``QkDAG``.
 */
QkTranspileLayout *qk_transpiler_pass_split_2q_unitaries(QkDag *dag,
                                                         double requested_fidelity,
                                                         bool split_swaps);
 ...
 */
void qk_transpile_state_layout_set(QkTranspilerStageState *state,
